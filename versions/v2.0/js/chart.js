...
const Chart = (() => {
  ...
  const MUSIC = {
    ...
    swag3: {
      wave: 'triangle',
      style: 'rnb',                               // R&B 编曲分支：808 + 摇摆镲 + 激光合成器
      chords: [                                   // Am7 - Fmaj7 - Cmaj7 - G6，慵懒七和弦进行
        { r: 45, pad: [57, 60, 64] },
        { r: 41, pad: [57, 60, 65] },
        { r: 48, pad: [55, 60, 64] },
        { r: 43, pad: [55, 59, 62] },
      ],
      lead: {                                     // 切分、大量空拍，slow-burning 勾线
        intro: [[_,_,_,_, 69,_,_,_, _,_,72,_, 69,_,_,_]],
        verse: [
          [69,_,_,_, 72,_,69,_, _,_,74,_, 72,_,_,_],
          [69,_,_,_, 76,_,74,_, 72,_,69,_, _,_,_,_],
          [72,_,_,_, 76,_,77,_, _,_,76,_, 74,_,72,_],
          [74,_,_,_, 72,_,69,_, 67,_,_,_, _,_,_,_],
        ],
        chorus: [
          [81,_,79,_, 76,_,_,79, 81,_,79,_, 76,_,_,_],
          [84,_,_,81, 79,_,81,_, 79,_,76,_, 74,_,_,_],
          [81,_,79,76, 79,_,_,_, 81,_,84,_, 81,_,79,_],
          [76,_,74,_, 72,_,74,76, 74,_,72,_, 69,_,_,_],
        ],
        outro: [[69,_,_,_, _,_,_,_, 64,_,_,_, _,_,_,_]],
      },
    },
  };
  ...
  const SONGS = [
    ...
    {
      id: 'swag3', name: '雾蓝 Swag', en: 'SWAG III · Homage',
      bpm: 92, emoji: '🌫️', desc: '雾蓝夜色 · 慢热 R&B，慢歌高精度挑战',
      speeds: { easy: 430, normal: 495, hard: 565 },
      music: MUSIC.swag3,
      palette: {
        sky: ['#0a0c14', '#1b2033'], sun: '#8f98ad', cloudA: 0.12, night: true,
        hillFar: '#181c2c', hillNear: '#121524',
        grass: '#262c44', grassLit: '#303756', grassDark: '#1e2338',
        dirt: '#101220', dirtDark: '#0b0d18',
        treeCap: ['#6f7686', '#4a5060'], trunk: '#aab1c0',
      },
    },
  ];
  ...
  /* 模式库：每小节事件 = [拍位(0~3.75), 车道 'G'/'A', 类型 'n'普通/'h'长按, 长按持续拍数]
     长按菇霸占据本车道 [拍位, 拍位+时长]，同车道该区间内不能排其他音符 */
  const POOLS = {
    easy: {
      verse: [
        [[0,'G','n'],[2,'G','n']],
  ...
  const DIFFS = {
    easy:   { label: '轻松', candyChance: 0.50 },
    normal: { label: '普通', candyChance: 0.60 },
    hard:   { label: '狂热', candyChance: 0.68 },
  };
  ...
  function generate(song, diffKey) {
    ...
    for (let bar = 0; bar < BARS; bar++) {
      ...
      for (const ev of pat) {
        ...
        if (ev[2] === 'h') {
          // 长度随机缩短一截：每只菇霸手感都不一样
          const cut = [0, 0.5, 1][Math.floor(Math.random() * 3)];
          events.push({ beat: beatPos, track: ev[1], kind: 'hold', dur: Math.max(1.25, ev[3] - cut) });
        } else {
          events.push({ beat: beatPos, track: ev[1], kind: 'normal' });
        }
      }
    }
    ...
    const candies = [];
    // 前奏给 3 颗定节奏的糖（互动收集，密度不能太高）
    for (const b of [4, 8, 12])
      candies.push({ beat: b, track: Math.random() < 0.5 ? 'A' : 'G' });
    for (let bar = 4; bar < BARS; bar++) {
      for (const off of [1.5, 2.5, 3.5]) {
        if (Math.random() > d.candyChance) continue;
        const beat = bar * 4 + off;
        const t = Math.random() < 0.5 ? 'A' : 'G';
        const conflict = clean.some(e =>
          Math.abs(e.beat - beat) < 0.3 ||
          // 长按菇霸占据本车道一段时间，糖不能落在它身上
          (e.kind === 'hold' && e.track === t && beat > e.beat - 0.3 && beat < e.beat + (e.dur || 0) + 0.3));
        if (conflict) continue;
        candies.push({ beat, track: t });
      }
    }
